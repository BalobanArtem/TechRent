...
async function loadFilters() {
  try {
    ...
    brandFilter.innerHTML = '<option value="all">Усі бренди</option>';
    ...
    typeFilter.innerHTML = '<option value="all">Усі категорії</option>';
...
function renderEquipment(equipment) {
  ...
  if (equipment.length === 0) {
    catalog.innerHTML = `
      <div style="grid-column: 1/-1; text-align: center; padding: 60px; color: #b79cff;">
        ...
        <p style="margin-top: 10px; color: #8181a0;">Спробуйте змінити параметри пошуку або скинути фільтри</p>
      </div>
    `;
    return;
  }
  ...
  catalog.innerHTML = equipment.map(item => {
    ...
    const descriptions = {
      'Пароконвектоматы': 'Професійне обладнання для приготування їжі з парою',
      'Холодильное оборудование': 'Надійне холодильне обладнання для зберігання продуктів',
      'Тепловое оборудование': 'Потужне теплове обладнання для професійної кухні',
      'Тестомесильное оборудование': 'Якісне обладнання для заміса тіста',
      'Фритюрницы': 'Професійні фритюрниці для смаження',
      'Планетарные миксеры': 'Потужні планетарні міксери для кондитерських',
      'Слайсеры и куттеры': 'Професійне обладнання для нарізки продуктів',
      'Посудомоечные машины': 'Промислові посудомийні машини для кафе та ресторанів'
    };
    
    const description = descriptions[item.type_name] || 'Якісне обладнання для професійного використання';
      ...
      <div class="card" data-id="${item.equipment_id}">
        ...
        <div class="card-content">
          ...
          <div class="price-block">
            ...
            ${item.for_sale === 'yes' ? `
              <div class="price-item">
                <span class="price-label">Купівля</span>
